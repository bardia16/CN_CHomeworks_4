...
int main() {
    ...
    for (auto& conn : connections) {
        index += 1;
        std::cout<< "After rtt update"<<std::endl;
        std::cout << "CWND: " << conn.getCwnd() << ", ssthresh: " << conn.getSsthresh() << ", RTT: " << conn.getRTT() << std::endl;
        ...
        // Simulate sending data again, but with reduced congestion window and slow start threshold for each connection
        std::cout << "Sending data again using reduced congestion window and slow start threshold for connection " << index << "...\n";
        conn.SendData();
        std::cout << "CWND: " << conn.getCwnd() << ", ssthresh: " << conn.getSsthresh() << ", RTT: " << conn.getRTT() << std::endl;
    }
